...
async function loadHotRounds() {
  ...
  const shouldUseApiProxy = params.get("api") === "1" || (!isStaticLocal && !isGitHubPages);
  try {
...
function renderCreatorInsight(index, title, weakness) {
  const insightMap = {
    ...
    平衡话术: {
      angle: "大家都听腻了安全正确，真正有传播力的是指出被平衡话术遮住的代价。",
...
} else if (bootParams.get("result") === "1" || bootParams.get("mode") === "result") {
  showDemoResult();
